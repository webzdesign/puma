        ...
        ->addColumn('action', function($row){
            ...
            if ($row->status == '0') {
                if (auth()->user()->hasPermission('activeinactive.users')) {
                    $actions .= " <a id='activate' href='" . $activeUrl . "' class='btn btn-success btn-sm activeUser'><i class='fa fa-check'></i> Activate</a>";
                }
            } else {
                if (auth()->user()->hasPermission('activeinactive.users')) {
                $actions .= " <a id='inactivate' href='" . $InactiveUrl . "' class='btn btn-danger btn-sm inactiveUser'><i class='fa fa-times'></i> In-activate</a>";
                }
            }
            if (auth()->user()->hasPermission('destroy.users')) {
                $actions .= " <a id='deleteuser' href='" . $deleteUrl . "' class='btn btn-danger  btn-sm'><i class='fas fa-trash'></i> Delete</a>";
            }
